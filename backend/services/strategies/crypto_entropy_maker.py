...
class CryptoEntropyMakerStrategy(BaseStrategy):
    ...
    default_config = {
        ...
        "min_confidence": 0.40,
        # H(0.80)=0.722, so 0.82 was mathematically incompatible with the
        # default min_entry_price=0.80. Keep the knob, disabled by default.
        "min_entropy": 0.0,
    ...
    def _score_market(self, row: dict[str, Any], cfg: dict[str, Any]) -> dict[str, Any] | None:
        ...
        min_entropy = max(0.0, min(1.0, to_float(cfg.get("min_entropy", 0.0), 0.0)))
        ...
        if cancel_recovery is not None:
            confidence = clamp(confidence + clamp(cancel_recovery * 0.12, 0.0, 0.08), 0.30, 0.92)
        if flow_imbalance is not None:
            confidence = clamp(confidence + clamp(flow_imbalance * 0.08, 0.0, 0.05), 0.30, 0.92)
        if recent_move_zscore is not None:
            confidence = clamp(
    ...
    def evaluate(self, signal: Any, context: dict[str, Any]) -> StrategyDecision:
        ...
        min_entropy = max(0.0, min(1.0, to_float(params.get("min_entropy", 0.0), 0.0)))
