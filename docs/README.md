...
| [`fix-08-undeclared-config-keys.md`](./fix-08-undeclared-config-keys.md) | **VOID — not implemented.** Four `directional_*` entry-price keys read via `params.get(key, magic)` without being declared. Real, but cosmetic: a trader runs one strategy on one source, so the inline default is reachable by tuning that strategy's params. Folded into the fix-13 config-key sweep. | historical context only. |
| [`fix-09-spread-gate-by-hold-intent.md`](./fix-09-spread-gate-by-hold-intent.md) | **VOID — not implemented.** Proposed tiering the spread gate by hold intent. The audit showed the premise was wrong: `max_entry_premium_pct` is exactly `max_spread_bps/200`, so the tiering already exists, and the gate was correct as written. `max_spread_bps` is a **config** decision (75 → 150–250), not a code change. | historical context only — read fix-10 instead. |
| [`rebuild-verified-base.md`](./rebuild-verified-base.md) | **The base itself.** Why `claude/rebuild-verified-base` was rebuilt from the clean source, and the disposition of all 17 code commits from the old branch — carried, corrected, or dropped, each with its reason. Records what is still outstanding rather than pretending it is solved. | before trusting any behaviour on this base, or asking why a change from the old branch is not here. |
| [`fix-10-forward-plan.md`](./fix-10-forward-plan.md) | **START HERE.** Forward plan replacing the revert-driven backlog: fix-10 signal-funnel attribution (per-gate rejection counts — the data already exists in `DecisionCheck.key` and was never aggregated), fix-11 sports has no producer at all, fix-12 news fetch fails silently (`TimeoutError` stringifies to `''`). Says explicitly what is *not* in scope and why. | any "why is this source 0" or "why did N signals produce 0 orders" question. |
