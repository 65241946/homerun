...
| [`fix-09-spread-gate-by-hold-intent.md`](./fix-09-spread-gate-by-hold-intent.md) | The global 75bps spread gate treats every strategy as round-trip, so hold-to-resolution bets (copy-trade, confluence, sports) are blocked on markets that are simply wide. Spread is a one-way premium there, not a round-trip cost. | touching `max_spread_bps`, or explaining why a high-conviction copy-trade signal won't fill. |
